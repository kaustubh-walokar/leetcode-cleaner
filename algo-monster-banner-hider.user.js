...
(function () {
  ...
  function toggleBanner() {
    ...
    // Use helper function to find the appropriate header
    banner = findHeader();

    // If we found a banner, toggle its visibility
    if (banner) {
      // Store the original state - check if banner is explicitly hidden
      const isHidden = banner.style.display === "none";

      // Toggle the banner visibility - use display property with more robust approach
      if (isHidden) {
        // Restore the banner by setting display to initial and removing any overrides
        banner.style.display = "";
        // Also reset other potential hiding properties
        banner.style.opacity = "";
        banner.style.visibility = "";
      } else {
        // Hide the banner by setting display to none
      ...
      if (toggleBtn) {
        ...
        toggleBtn.textContent = isHidden
          ? CONFIG.EMOJI_VISIBLE
          : CONFIG.EMOJI_HIDDEN;
      }
    }
  }

  // Helper function to find header or top navigation bar based on position
  function findHeader() {
    // Try to find the header or top navigation bar with specific selectors
    const headers = document.querySelectorAll("header, .sticky-top");
